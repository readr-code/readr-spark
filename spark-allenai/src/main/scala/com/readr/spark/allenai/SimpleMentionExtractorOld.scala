...
object SimpleMentionExtractorOld {
  
}

class SimpleMentionExtractorOld extends Annotator(
      generates = Array(classOf[NERAnn]), //, classOf[MentionAnn]),
  ...
  def run(ta:TextAnn, toa:TokenOffsetAnn, stoa:SentenceTokenOffsetAnn, sda:SentenceDependencyAnn, pa:POSAnn):NERAnn = {

    // identify NP chunks according to patterns
	val l = new ArrayBuffer[NamedEntity]()
    ...
    for (sentNum <- 0 until stoa.sents.size) {
      ...
      while (tokStart < so.t) {
        ...
        if (tokEnd > tokStart)
          l += NamedEntity("unknown", Offsets(tokStart, tokEnd), tokEnd-1)
        tokStart = tokEnd + 1
      }
    }
	NERAnn(l.toArray)
